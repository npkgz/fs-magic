...
describe('md5sum', function(){
    ...
    it('should throw an exepction in case the file is not available (ENOENT)', function(){
       ...
       return _fs.md5file(tmpDir + '/unkownFile.txt').then(function(result){
    ...
    it('should calculate md5sum as hex case the output format is unknown', function(){
        
        return _fs.md5file(_testfile, 'unknownformat').then(function(result){
            _assert.equal(result, _refChecksums.hex.md5);
        });
    });

    it('should calculate md5sum as hex (default)', function(){
        return _fs.md5file(_testfile).then(function(result){
            _assert.equal(result, _refChecksums.hex.md5);
        });
    });
    
    it('should calculate md5sum as base64 (forced)', function(){
        return _fs.md5file(_testfile, 'base64').then(function(result){
