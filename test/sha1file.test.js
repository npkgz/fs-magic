...
describe('sha1sum', function(){
    ...
    it('should throw an exepction in case the file is not available (ENOENT)', function(){
       ...
       return _fs.sha1file(tmpDir + '/unkownFile.txt').then(function(result){
    ...
    it('should calculate sha1sum as hex case the output format is unknown', function(){
        
        return _fs.sha1file(_testfile, 'unknownformat').then(function(result){
            _assert.equal(result, _refChecksums.hex.sha1);
        });
    });

    it('should calculate sha1sum as hex (default)', function(){
        return _fs.sha1file(_testfile).then(function(result){
            _assert.equal(result, _refChecksums.hex.sha1);
        });
    });
    
    it('should calculate sha1sum as base64 (forced)', function(){
        return _fs.sha1file(_testfile, 'base64').then(function(result){
